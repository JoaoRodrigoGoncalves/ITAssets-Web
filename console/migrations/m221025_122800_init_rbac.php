...
class m221025_122800_init_rbac extends Migration
{
    ...
    public function up()
    {
        ...
        $auth->addChild($funcionario, $CriarAlocacaoItem);
        $auth->addChild($funcionario, $CriarReparacaoItem);
        $auth->addChild($funcionario, $ConsultarPedidoAlocacaoProprio);
        $auth->addChild($funcionario, $ConsultarPedidoReparacaoProprio);
        $auth->addChild($funcionario, $EditarAlocacaoItem);
        $auth->addChild($funcionario, $EditarReapacaoItem);
        $auth->addChild($funcionario, $CancelarAlocacaoItem);
        $auth->addChild($funcionario, $CancelarReparacaoItem);
        $auth->addChild($funcionario, $ConsultarListaItems);
        $auth->addChild($funcionario, $ConsultarDetalhesItem);
        $auth->addChild($funcionario, $ConsultarPropriaConta);
        $auth->addChild($funcionario, $EditarPropriaConta);

        // Cria o utilizador "Utilizador Logistica"
        $logistica = $auth->createRole("operadorLogistica");
        $auth->add($logistica);
        $auth->addChild($logistica, $funcionario);
